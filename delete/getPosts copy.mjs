...
import { apiUrl } from '../js/api/auth/apiBase.mjs';
import { apiGetPosts } from '../js/api/auth/apiBase.mjs';
import { sortCreatedDesc } from '../js/api/auth/apiBase.mjs';
import { sortCreatedAsc } from '../js/api/auth/apiBase.mjs';
/* import { newFilter } from './filterPost.js';
import { oldFilter } from './filterPost.js'; */
    ...
    if (response.ok === true) {
      for (let i = 0; i < requestedPosts.length; i++) {
        ...
        newestPostFilter.onclick = function () {
          const newAPI = `${apiUrl}${apiGetPosts}?${sortCreatedDesc}&_author=true&limit=30`;
          postsBox.innerHTML = `${importCard}`;
          requestPost(newAPI);
          console.log(newAPI);
        };
        ...
        oldestPostFilter.onclick = function () {
          const oldAPI = `${apiUrl}${apiGetPosts}?${sortCreatedAsc}&_author=true&limit=30`;
          postsBox.innerHTML = `${importCard}`;
          requestPost(oldAPI);
          console.log(oldAPI);
        };
